...
describe("ETagExtractorHeaderStrategy", () => {
  ...
  test("invalid header - empty", () => {
    const context = new RequestContextBuilder().withHeader(tools.ETag.IF_MATCH_HEADER_NAME, "").build();

    expect(strategy.detect(context)).toEqual(null);
  });

  test("invalid header - blank", () => {
    const context = new RequestContextBuilder().withHeader(tools.ETag.IF_MATCH_HEADER_NAME, " ").build();

    expect(strategy.detect(context)).toEqual(null);
  });

  test("invalid header - blank - not normalized", () => {
    const context = new RequestContextBuilder().withRawHeader(tools.ETag.IF_MATCH_HEADER_NAME, " ").build();
